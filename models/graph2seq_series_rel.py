...
from models.VAR.var_dec import VariationalDecoder
from models.VAR.utils import tile
...
class Graph2SeqSeriesRel(nn.Module):
    def __init__(self, args, vocab: Dict[str, int]):
        ...
        self.decoder = VariationalDecoder(args,
    ...
    def encode_and_reshape(self, reaction_batch: G2SBatch):
        ...
        product_emb = padded_memory_bank
        ...
        return padded_memory_bank, memory_lengths, product_emb

    def forward(self, reaction_batch: G2SBatch):
        padded_memory_bank, memory_lengths, product_emb = self.encode_and_reshape(reaction_batch)
        ...
        dec_outs, _, aux_loss, kld_loss = self.decoder(product_emb,
            tgt=dec_in,
        ...
        return (loss, aux_loss, kld_loss), acc
    ...
    def predict_step(self, reaction_batch: G2SBatch,
        ...
        padded_memory_bank, memory_lengths, product_emb = self.encode_and_reshape(reaction_batch=reaction_batch)
        ...
        product_emb = tile(product_emb, beam_size, dim=1)

        # (3) Begin decoding step by step:
        for step in range(decode_strategy.max_length):
            decoder_input = decode_strategy.current_predictions.view(1, -1, 1)
            dec_out, dec_attn = self.decoder(product_emb,
                                             tgt=decoder_input,
            ...
            if any_finished:
                ...
                if isinstance(memory_bank, tuple):
                    ...
                    product_emb = product_emb.index_select(1, select_indices)
        ...
        if hasattr(self.decoder, 'y_sample'):
            return results, self.decoder.y_sample.argmax(-1).reshape(-1, beam_size)
        else:
            return results

...
class G2SBatch:
    def __init__(self,
                ...
                #  N_tgt: torch.Tensor = None,
                #  N_tgt_lens: torch.Tensor = None):
        self.fnode = fnode
        self.fmess = fmess
        self.agraph = agraph
        self.bgraph = bgraph
...
class G2SDataset(Dataset):
    def __init__(self, args, file: str, unique=False):
        self.args = args
        ...
        #++++++++++++++++++++
        save_dir = f"./data/{args.data_name}" #  './data/miniFULL'
        if 'train' in file:
            fp = os.path.join(save_dir, "src-train.txt")
        elif 'val' in file:
            fp = os.path.join(save_dir, "src-val.txt")
        elif 'test' in file:
            fp = os.path.join(save_dir, "src-test.txt")
        with open(fp, 'r') as f:
            ptr = []
            last_sp = None
            lines = f.readlines()
    ...
    def __getitem__(self, index: int) -> G2SBatch:
        ...
        #++++++++++++++++++++++++
        pres, posts = [], []
        for id in data_indices:  #torch.as_tensor(data_indices, dtype=torch.long)
        ...
        tgt_ids = torch.as_tensor(self.tgt_token_ids, dtype=torch.long)
        ts=[tgt_ids[torch.arange(pres[i], posts[i]), :max(tgt_lengths)] for i in range(len(data_indices))] ## list, Sequence of unequal lengths
        for i, new_ts in enumerate(ts):
            old_t = tgt_token_ids[i]
            uni_idx = (new_ts!=old_t).float().argmax(-1)  # len of len(new_ts) the first index of each row that new&old differs
        ...
        g2s_batch = G2SBatch(
            ...
            distances=distances,
            #++++++++++++++++++++++++
            data_indice=torch.as_tensor(data_indices, dtype=torch.long),
            multi_tgts=ts,
            pres=pres,
            posts=posts
            # N_tgt=N_tgt,
